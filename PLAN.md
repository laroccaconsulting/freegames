...
- **Logic** app ✅ Built — `games/logic/`: one app for these puzzles (modes share daily, saving, results). **Star Battle** ✅ (6×6, 8×8, 10×10 two-star; regions repaired until the solver proves one answer; automatic dots, hints). **Calcudoku** ✅ (4×4 to 6×6; random Latin square cut into cages, kept when the solver proves one answer; notes, hints). **Bridges** ✅ (7×7 to 10×12; islands grown as a tree of bridges, solver with bound propagation proves one answer; drag or tap to build). **Futoshiki** ✅ and **Skyscrapers** ✅ (4×4 to 6×6; shared Latin-square solver in `js/latin.js`, clues thinned while the answer stays unique within a small search budget; shared number-pad view). **Light Up** ✅ (7×7 to 11×11; symmetric black squares, bulbs dropped until all lit, numbers thinned while a lit-square search proves one answer) and **Tents** ✅ (6×6 to 10×10; tents paired with trees, row/column counts, search with bipartite matching proves one answer). **Kakuro** ✅ (6×6 to 9×9; every white square in an across and a down run, digits repaired where two answers disagree until the sums alone pin it down, no givens)
- **Star Battle** (not "Queens"), **Calcudoku** (not "KenKen"), Kakuro ✅, Futoshiki ✅, Skyscrapers ✅
- **Bridges** (Hashi) ✅, Slitherlink, Nurikabe, Light Up ✅, Tents ✅, **Pipes** ✅ (own app, `games/pipes/`: random spanning tree scrambled, par = fewest turns, daily + four sizes, water shows what is connected)
...
- **Nine Men's Morris** ✅ Built — `games/morris/` (place, move, fly; mills take pieces; alpha-beta computer at three depths; pass-and-play; 50-move draw), 9×9 Go
