// Synthesized sounds for Gems. Chimes use the current theme's scale.
...
export const sfx = {
  swap() {
    const ac = audio();
    if (!ac) return;
    noiseBurst(ac, { duration: 0.06, freq: 2400, q: 0.8, gain: 0.12 });
  },
  // A light clack when a gem is picked.
  tap() {
    const ac = audio();
    if (!ac) return;
  ...
  // Gems landing after a fall.
  land() {
    const ac = audio();
    if (!ac) return;
    noiseBurst(ac, { duration: 0.03, freq: 900, q: 1.2, gain: 0.14 });
  },
  blocked() {
    const ac = audio();
    if (ac) tone(ac, { freq: 180, duration: 0.1, gain: 0.06, type: 'triangle' });
  },
  // A line clears: the chord climbs with each step of a cascade.
  match(combo = 0) {
    const ac = audio();
    if (!ac) return;
